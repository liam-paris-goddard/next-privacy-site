  ...
  <ul className="gsi-footer__legal">
    ...
    {legalLinksList.map(item =>
      <li key={item.targetUrl}>
        <a id={item.id ? item.id : ""} className="gsi-footer__link gsi-footer__link--legal" href={item.targetUrl} target={item.newTab ? '_blank' : ''} rel={item.newTab ? 'noopener noreferrer' : ''}>
          {item.linkText}
        </a>
      </li>
    )}
